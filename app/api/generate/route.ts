  ...
  try {
    ...
    if (!isValidInput(input)) {
      logEvent("warn", {
        ...
        category: input.presetId,
      });
      return NextResponse.json(
        { error: "플랫폼, 주제, 출력 설정을 확인해 주세요.", requestId },
        { status: 400, headers: { "x-blotori-request-id": requestId } },
      );
    }
    ...
    logEvent("info", {
      ...
      category: input.presetId,
      model: process.env.OPENAI_MODEL || "gpt-5.6-luna",
      ragEnabled: grounding?.enabled ?? false,
      ragUsed: grounding?.used ?? false,
      ragResultCount: grounding?.resultCount ?? 0,
      ragSourceCount: grounding?.sourceNames?.length ?? 0,
    });

    if (grounding?.enabled && !grounding.used) {
      logEvent("warn", {
        event: "rag.empty_result",
        requestId,
        durationMs: Date.now() - startedAt,
        status: "warning",
        category: input.presetId,
        ragEnabled: true,
        ragUsed: false,
        ragResultCount: grounding.resultCount,
        ragSourceCount: grounding.sourceNames.length,
        message: "RAG가 활성화됐지만 검색 근거가 사용되지 않았습니다.",
      });
    }

    return NextResponse.json(
      { ...result, requestId },
      { headers: { "x-blotori-request-id": requestId } },
    );
  } catch (error) {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "생성 중 오류가 발생했습니다.", requestId },
      { status: 500, headers: { "x-blotori-request-id": requestId } },
    );
  }
}
