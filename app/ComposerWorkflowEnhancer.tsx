...
function detectPlatform(): PlatformId | null {
  const select = Array.from(document.querySelectorAll<HTMLSelectElement>("select")).find((item) =>
    platformIds.includes(item.value as PlatformId),
  );
...
function paperSignature(paper: HTMLElement) {
  return [
    paper.querySelector(".previewTitle")?.textContent ?? "",
    paper.querySelectorAll(".articleSection").length,
    paper.querySelectorAll(".textEmphasis").length,
    paper.querySelectorAll(".imageSlot").length,
  ].join("|");
}
...
function clickExistingButton(pattern: RegExp) {
  const button = Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find(
    (item) => pattern.test(item.textContent ?? "") && !item.closest(".composerWorkflowEnhancer"),
  );
  button?.click();
...
function buildRenderedGuide(paper: HTMLElement, platformId: PlatformId) {
  ...
  paper.querySelectorAll<HTMLElement>(".articleSection").forEach((section) => {
    ...
    section.querySelectorAll<HTMLElement>(".textEmphasis").forEach((node) => {
      ...
      if (!phrase) return;
      const kind = node.classList.contains("accent")
        ? "포인트 색 + 굵게"
  ...
  if (platformId === "naver") {
    ...
    lines.push(
      "이미지는 복붙하지 말고 각 IMG 위치에서 포토업로더로 원본 파일 등록",
      "태그는 본문에 붙이지 말고 태그 입력 영역에 별도로 등록",
    );
  }
...
export default function ComposerWorkflowEnhancer() {
  ...
  const scan = useCallback(() => {
    ...
    setRuntime((current) => {
      ...
      return current?.host === next.host &&
        current.paper === next.paper &&
        current.platformId === next.platformId &&
  ...
  const guideLines = useMemo(
    () => (runtime ? buildRenderedGuide(runtime.paper, runtime.platformId) : []),
    [runtime],
  );
